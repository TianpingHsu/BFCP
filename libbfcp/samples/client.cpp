...
int sampleClient::OpenParticipantTcpConnection(const char* local_address, int local_port, const char* remote_address, int remote_port, bool isPassive ) {
    ...
    try {
        ...
        if ( m_BFCP_Participant == NULL  )
            m_BFCP_Participant = new BFCP_Participant( m_confID , m_userID , m_floorID , m_streamID , this , BFCP_OVER_UDP);
        ...
        if ( m_BFCP_Participant ) {
            ...
            if ( m_BFCP_Participant->OpenUdpConnection( local_address,local_port,remote_address,remote_port)) {
...
static int client_localport = 7000;
...
void sampleClient::menu(char *lineptr) {
	...
	printf("Enter the FloorID:\n");
            ...
            //scanf (" %a[^\n]", &text1);
            text1 = (char*)calloc(20, sizeof(char));
            ...
            BFCP_CHECK_INTEGER;
            ++lineptr;
            printf("Enter the local port of participant: \n");
            error = scanf ("%i", &client_localport);
            OpenParticipantTcpConnection("172.25.29.243", client_localport , "172.25.29.243", port_server , GetIsPassive() );
            free(text1);
            break;
        case 'd':
            ++lineptr;
