/**
 *
 * \brief Definitions of BFCP packet format and methodes for encoding and
 * decoding messages
 *
...
typedef void (*tLog_callback)(char *pcFile, int iLine, int iErrLevel,
                              char *pcFormat, ...);

/*!
 *      \brief  Affectation d'un pointeur de fonction
 *      \param  trace_function     Fonction attribuer aux system de trace PS
 * app. \retval void
 */
...
/* Maximum allow size for BFCP messages is 64Kbytes, since the Payload Length in
 * the header is 16 bit */
 ...
 *       receiver.
 *
 *       \subsection Primitive: This 8-bit field identifies the main purpose of
 * the message.  The following primitive values are defined
 *
 *       \li Payload Length: This 16-bit field contains the length of the
 * message in 4-octet units, excluding the common header.
 *
 *       \li  Conference ID: This 32-bit field identifies the conference the
 *       message belongs to.
 *
 *       \li Transaction ID: This field contains a 16-bit value that allows
 * users to match a given message with its response.  The value of the
 *       Transaction ID in server-initiated transactions is 0 (see Section 8).
 *
 *       \li User ID: This field contains a 16-bit value that uniquely
 ...
 *         +-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
 *
 *      \li Floor ID: This field contains a 16-bit value that uniquely
 ...
 *
 *       \li A floor request is in Pending state if the floor control server
 * needs to contact a floor chair in order to accept the floor request, but has
 * not done it yet.  Once the floor control chair accepts the floor request, the
 * floor request is moved to the Accepted state.
 *
 *
 ...
 *
 *       \li Reserved: At this point, the 13 bits in the reserved field SHOULD
 * be set to zero by the sender of the message and MUST be ignored by the
 *       receiver.
 *
 */
...
typedef struct {
    ...
    UINT16 TransactionID; /**     @brief  transaction ID of request  \ref
                             BFCP-PROTOCOL-TRANSACTION */
    UINT16
        userID; /**     @brief   user ID of request \ref BFCP-COMMON-HEADER */
    UINT32 conferenceID;   /**     @brief  conference ID of request \ref
                              BFCP-COMMON-HEADER  */
    UINT16 FloorID;        /**     @brief   floor id of request \ref FLOOR-ID */
    UINT16 FloorRequestID; /**     @brief  floor request id \ref
                              FLOOR-REQUEST-ID  */
    UINT16 QueuePosition; /**    @brief   queue position \ref  REQUEST-STATUS */
    UINT16 BeneficiaryID; /**     @brief   benficiary ID of request  \ref
                             BENEFICIARY-ID */
    BFCP_SOCKET
        sockfd;    /**     @brief  socket fille desc receving this request  */
    UINT16 i_parm; /**     @brief   integer for variables paramters  */
    char c_param[BFCP_STRING_SIZE]; /**     @brief  char table for or variables
                                       paramters    */
    void *pt_param; /**     @brief  pointer  or variables paramters   */
    UINT8 Support_primitives[BFCP_MAX_RG]; /**     @brief  binary table of
                                              supported primitive by FCS   \ref
                                              e_bfcp_primitives */
    UINT8 Support_attributes[BFCP_MAX_RG]; /**     @brief   binary table of
                                              supported attributes by FCS  \ref
                                              e_bfcp_attributes */
    int transport;
} s_bfcp_msg_event;
...
typedef struct bfcp_floor_id_list { /*   @brief   FLOOR-ID list, to manage the
                                       multiple FLOOR-ID attributes */
    UINT16 ID;                      /*   @brief   FLOOR-ID */
    struct bfcp_floor_id_list
        *next; /*    @brief   Pointer to next FLOOR-ID instance */
} bfcp_floor_id_list;

typedef struct bfcp_supported_list { /*   @brief   list to manage all the
                                        supported attributes and primitives */
    UINT8 element; /*   @brief   Element (Attribute/Primitive) */
    struct bfcp_supported_list
        *next; /*  @brief    Pointer to next supported element instance */
} bfcp_supported_list;
...
typedef struct bfcp_unknown_m_error_details { /*     These are specific details
                                                 for error 4: UNKNOWN_M */
    UINT16 unknown_type; /*     7-bits to specify the unknown received mandatory
                            attribute */
    UINT16 reserved; /*     A reserved bit, which must be set to 0 and ignored,
                        at the moment */
...
typedef struct bfcp_user_information { /*     Help structure for
...
typedef struct bfcp_floor_request_status { /*     Help structure for
    ...
    struct bfcp_floor_request_status
        *next; /*     pointer to next instance (to manage lists) */
} bfcp_floor_request_status;

typedef struct bfcp_overall_request_status { /*     Help structure for
                                                OVERALL-REQUEST-STATUS */
    UINT16 frqID;            /*     OVERALL-REQUEST-STATUS-HEADER */
    bfcp_request_status *rs; /*     REQUEST-STATUS, optional */
    char *sInfo;             /*     STATUS-INFO, optional */
#if HSU
    UINT16 floorID;          /*     FLOOR-REQUEST-STATUS, optional, default value is 0*/
#endif // HSU
} bfcp_overall_request_status;

typedef struct bfcp_floor_request_information { /*     Help structure for
    ...
    bfcp_user_information
        *beneficiary; /*     BENEFICIARY-INFORMATION, optional */
    bfcp_user_information
        *requested_by;        /*     REQUESTED-BY-INFORMATION, optional */
    e_bfcp_priority priority; /*     PRIORITY, optional */
    char *pInfo;              /*     PARTICIPANT-PROVIDED-INFO, optional */
    struct bfcp_floor_request_information
        *next; /*     pointer to next instance (to manage lists) */
...
typedef struct bfcp_received_attribute { /*     A chained list to manage all
    ...
    int valid; /*     If errors occur in parsing, the attribute is marked as not
                  valid */
    struct bfcp_received_attribute
        *next; /*     Pointer to next attribute in the message */
} bfcp_received_attribute;

typedef struct bfcp_received_message {
    bfcp_arguments
        *arguments; /*     The message unpacked in its original arguments */
    ...
    bfcp_received_attribute
        *first_attribute; /*     A list of all attributes in the message */
    struct bfcp_received_message_error
        *errors; /*     If errors occur, we write them here */
    int transport;
...
typedef struct bfcp_received_message_error {
    ...
    struct bfcp_received_message_error
        *next; /*     There could be more errors, it's a linked list */
...
bfcp_entity *bfcp_new_entity(UINT32 conferenceID, UINT16 transactionID,
                             UINT16 userID);
/*     Free an Entity */
int bfcp_free_entity(bfcp_entity *entity);

/*     Create a new Floor ID list (first argument must be a valid ID, last
 * argument MUST be 0) */
...
/*     Create a new Supported (Primitives/Attributes) list (last argument MUST
 * be 0) */
...
bfcp_error *bfcp_new_error(e_bfcp_error_codes code,
                           bfcp_unknown_m_error_details *details);
/*     Free an Error (Code/Details) */
int bfcp_free_error(bfcp_error *error);

/*     Create a New Error Details list (for Error 4: UNKNOWN_M) (last argument
 * MUST be 0) */
bfcp_unknown_m_error_details *bfcp_new_unknown_m_error_details_list(
    unsigned attribute, ...);
/*     Add Attributes to an existing Error Details list (for Error 4: UNKNOWN_M)
 * (last argument MUST be 0) */
int bfcp_add_unknown_m_error_details_list(bfcp_unknown_m_error_details *list,
                                          unsigned attribute, ...);
/*     Free an Error Details list */
int bfcp_free_unknown_m_error_details_list(
    bfcp_unknown_m_error_details *details);

/*     Create a New User (Beneficiary/RequestedBy) Information */
bfcp_user_information *bfcp_new_user_information(UINT16 ID, char *display,
                                                 char *uri);
...
bfcp_floor_request_information *bfcp_new_floor_request_information(
    UINT16 frqID, bfcp_overall_request_status *oRS,
    bfcp_floor_request_status *fRS, bfcp_user_information *beneficiary,
    bfcp_user_information *requested_by, e_bfcp_priority priority, char *pInfo);
...
int bfcp_list_floor_request_information(bfcp_floor_request_information *frqInfo,
                                        ...);
/*     Add elements to an existing Floor Request Information list (last argument
 * MUST be NULL) */
int bfcp_add_floor_request_information_list(
    bfcp_floor_request_information *list, ...);
/*     Free a Floor Request Information list */
int bfcp_free_floor_request_information_list(
    bfcp_floor_request_information *frqInfo);

/*     Create a New Floor Request Status
 * (FloorID/RequestStatus/QueuePosition/StatusInfo) */
bfcp_floor_request_status *bfcp_new_floor_request_status(UINT16 fID, UINT16 rs,
                                                         UINT16 qp,
                                                         char *sInfo);
...
/*     Add elements to an existing Floor Request Status list (last argument MUST
 * be NULL) */
int bfcp_add_floor_request_status_list(bfcp_floor_request_status *list, ...);
/*     Free a Floor Request Status list */
int bfcp_free_floor_request_status_list(
    bfcp_floor_request_status *floor_request_status);

/*     Create a New Overall Request Status
 * (FloorRequestID/RequestStatus/QueuePosition/StatusInfo) */
bfcp_overall_request_status *bfcp_new_overall_request_status(UINT16 frqID,
                                                             UINT16 rs,
                                                             UINT16 qp,
                                                             char *sInfo);
#if HSU
bfcp_overall_request_status* qn_bfcp_new_overall_request_status(UINT16 frqID,
    UINT16 rs,
    UINT16 qp,
    UINT16 floorID,
    char* sInfo);
...
int bfcp_free_overall_request_status(
    bfcp_overall_request_status *overall_request_status);
...
/*     Build Headers */
void bfcp_build_commonheader(bfcp_message *message, bfcp_entity *entity,
                             e_bfcp_primitives primitive, int unreliable);
void bfcp_build_attribute_tlv(bfcp_message *message, UINT16 position,
                              UINT16 type, UINT16 mandatory_bit, UINT16 length);

/*     Build Specific Messages */
bfcp_message *bfcp_build_message_FloorRequest(bfcp_entity *entity,
                                              bfcp_floor_id_list *fID,
                                              UINT16 bID, char *pInfo,
                                              e_bfcp_priority priority,
                                              int unreliable);
bfcp_message *bfcp_build_message_FloorRelease(bfcp_entity *entity, UINT16 frqID,
                                              int unreliable);
bfcp_message *bfcp_build_message_FloorRequestQuery(bfcp_entity *entity,
                                                   UINT16 frqID,
                                                   int unreliable);
bfcp_message *bfcp_build_message_FloorRequestStatus(
    bfcp_entity *entity, bfcp_floor_request_information *frqInfo,
    int unreliable);
bfcp_message *bfcp_build_message_UserQuery(bfcp_entity *entity, UINT16 bID,
                                           int unreliable);
bfcp_message *bfcp_build_message_UserStatus(
    bfcp_entity *entity, bfcp_user_information *beneficiary,
    bfcp_floor_request_information *frqInfo, int unreliable);
bfcp_message *bfcp_build_message_FloorQuery(bfcp_entity *entity,
                                            bfcp_floor_id_list *fID,
                                            int unreliable);
bfcp_message *bfcp_build_message_FloorStatus(
    bfcp_entity *entity, bfcp_floor_id_list *fID,
    bfcp_floor_request_information *frqInfo, int unreliable);
bfcp_message *bfcp_build_message_ChairAction(
    bfcp_entity *entity, bfcp_floor_request_information *frqInfo,
    int unreliable);
...
bfcp_message *bfcp_build_message_HelloAck(bfcp_entity *entity,
                                          bfcp_supported_list *primitives,
                                          bfcp_supported_list *attributes);
bfcp_message *bfcp_build_message_Error(bfcp_entity *entity, bfcp_error *error,
                                       char *eInfo, int unreliable);
...
int bfcp_build_attribute_PRIORITY(bfcp_message *message,
                                  e_bfcp_priority priority);
int bfcp_build_attribute_REQUEST_STATUS(bfcp_message *message,
...
int bfcp_build_attribute_PARTICIPANT_PROVIDED_INFO(bfcp_message *message,
                                                   char *pInfo);
int bfcp_build_attribute_STATUS_INFO(bfcp_message *message, char *sInfo);
int bfcp_build_attribute_SUPPORTED_ATTRIBUTES(bfcp_message *message,
                                              bfcp_supported_list *attributes);
int bfcp_build_attribute_SUPPORTED_PRIMITIVES(bfcp_message *message,
                                              bfcp_supported_list *primitives);
int bfcp_build_attribute_USER_DISPLAY_NAME(bfcp_message *message,
                                           char *display);
int bfcp_build_attribute_USER_URI(bfcp_message *message, char *uri);
int bfcp_build_attribute_BENEFICIARY_INFORMATION(
    bfcp_message *message, bfcp_user_information *beneficiary);
int bfcp_build_attribute_FLOOR_REQUEST_INFORMATION(
    bfcp_message *message, bfcp_floor_request_information *frqInfo);
int bfcp_build_attribute_REQUESTED_BY_INFORMATION(
    bfcp_message *message, bfcp_user_information *requested_by);
int bfcp_build_attribute_FLOOR_REQUEST_STATUS(bfcp_message *message,
                                              bfcp_floor_request_status *fRS);
int bfcp_build_attribute_OVERALL_REQUEST_STATUS(
    bfcp_message *message, bfcp_overall_request_status *oRS);
