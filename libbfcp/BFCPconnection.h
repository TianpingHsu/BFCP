...
#define BFCP_FCS_DEFAULT_PORT \
    2345 /** @brief The default port the Floor Control Server will bind to */
#define BFCP_MAX_CONNECTIONS                                                   \
    1000 /** @brief  The default value for how many connections the server can \
...
class BFCPConnection {
   public:
     ...
     */
    virtual bool OnBFCPConnected(BFCP_SOCKET socket, const char* RemoteIp,
                                 int remotePort) = 0;
    /**
    ...
    /**
     * \brief Virtual log and traces callback , for better traces integration on
     * your process
     * @param pcFile file reference
     * @param iLine  line refernce
     * @param iErrorLevel error level \ref INF debug \ref WAR warning \ref ERR
     * error
     * @param pcFormat char formated traces
     */
    virtual void Log(const char* pcFile, int iLine, int iErrorLevel,
     ...
     * @param localAddress local ip local IP V4 address,  you can use NULL to
     ...
     * @param message BFCP message
     * @param donotresend true: this should not be resent (on unreliable
     *transport). false: manage retransmission
     *
     * @return  1 = success
     *          0 = could not send transport error
     *	       -1 = duplicate transaction ID
     */
    int sendBFCPmessage(BFCP_SOCKET client_sock, bfcp_message* message,
   ...
   protected:
    /**
     * Add a new client. Can be active, passive TCP or TLS client. Can be UDP
     *client. WARNING : only UDP client are supported for the moment.
     * @param transport to use for BCP protocol
     **/
    BFCP_SOCKET AddClient(int transport, int role, char* localAddress = NULL,
                          UINT16 port = 0);
    ...
    bool SetRemoteAddressAndPort(BFCP_SOCKET s, const char* remoteIp,
                                 UINT16 remotePort);
   ...
   private:
    ...
    class Transaction {
       ...
       public:
        bfcp_message* message; /* <! message that created the transaction and
                                * need to be resent if no ack is received this
                                * can also be the answer to resend in case of
                                * retranmission of a request by a remote party
                                */
    ...
    class Client2ServerInfo {
       ...
       public:
        ...
        Client2ServerInfo(int transport, int role = BFCPConnectionRole::PASSIVE)
            : m_role(role), m_bfcp_transport(transport) {
            Init();
        }
        ...
        inline void SetRemoteAddres(struct sockaddr_storage* addr,
                                    socklen_t addrlen) {
        ...
        const char* GetRemoteAddrAndPort() {
            return m_remoteAddressAndPort.c_str();
        }
         ...
         **/
        bool HasReliableTransport() {
            return GetTransport() == BFCP_OVER_TCP ||
                   GetTransport() == BFCP_OVER_TLS;
        }
        ...
        bool HandleRemoteRetrans(BFCPConnection* c, BFCP_SOCKET s,
                                 bfcp_message* m);

        /**
         * Check answer expiration for unreliable transport and cal on
         *OnBFCPDisconnected if this was a GoodByeAck answer
         *
         * @return 1 : at lease one transaction has expired. Rerun. 0 : no
         *transaction expired -1 - special case where GoodByeAck transaction has
         *expired.
         *
         **/
       ...
       private:
        /**
         * Utility method that translate and IP address / port into a sockaddr
         *struct
         *
         **/
        static bool SetAddress(const char* addrstr, UINT16 port,
