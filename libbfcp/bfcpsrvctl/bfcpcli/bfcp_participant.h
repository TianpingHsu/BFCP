...
#if HSU
    /**
     * Open a new UDP connection with BFCP server .
     * if the connection success , api send BFCP hello message .
     * On the hello message this class sets the Conference ID and the
     * Transaction ID in the common header . API sets the User ID in the common
     * header to the client's identifier. This User ID will be used by the floor
     * control server to authenticate and authorize the request. the respons of
     * Floor control server are recieved on \ref
     * ParticipantEvent::OnBfcpParticipantEvent
     * @param local_address local ip to use , NULL if you want use all ip
     * interface .
     * @param local_port  local ip port , 0 if you dont want use specific port
     * @param remote_address  ip adress of distant
     * @param remote_port   port of distant.
     * @return true sucess , if you are on server mode , the server is launched
     * and waitting connection , else on client mode , you are connected of
     * remote server .
     */
    bool OpenUdpConnection(const char* local_address, int local_port,
                           const char* remote_address, int remote_port);
    /**
     * Close current UDP connection .
     * @return true sucess , false failed .
     */
    bool CloseUdpConnection();
#endif  // HSU
