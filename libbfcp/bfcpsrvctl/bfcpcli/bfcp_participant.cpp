...
extern "C" void _ParticpantLog(char *pcFile, int iLine, int iErrorLevel,
                               char *pcFormat, ...) {
...
BFCP_Participant::BFCP_Participant(
    UINT32 p_confID, UINT16 p_userID, UINT16 p_floorID, UINT16 p_streamID,
    BFCP_Participant::ParticipantEvent *p_ParticipantEvent, int transport)
    ...
    if (p_confID == 0)
        throw IllegalArgumentException(
            "BFCP_Participant:: conferenceID invalid");
    if (p_userID == 0)
    ...
    struct_participant = (st_bfcp_participant_information *)calloc(
        1, sizeof(st_bfcp_participant_information));
    if (struct_participant == NULL)
        throw IllegalArgumentException(
            "BFCP_Participant:: Allocate and setup invalid");

    Log(INF,
        "BFCP_Participant:: created participant conferenceID[%d] userID[%d] "
        "floorID[%d] stramID[%d]",
...
BFCP_Participant::~BFCP_Participant(void) {
    Log(INF, _T("BFCP destroy participant."));
    removeSession();
...
#if HSU
void BFCP_Participant::initStateMachine() {
    ...
    st_BFCP_fsm BFCPStateMachine[BFCP_ACT_LAST][BFCP_ACT_LAST] = {
        ...
        /* from BFCP_ACT_CONNECTED */
        {
            ...
            {BFCP_ACT_CONNECTED,
             &BFCP_fsm::
                 BFCPFSM_FloorRequestStatusAccepted}, /* BFCP_ACT_FloorRequestStatusAccepted*/
            {BFCP_ACT_CONNECTED,
             &BFCP_fsm::
                 BFCPFSM_FloorRequestStatusAborted}, /* BFCP_ACT_FloorRequestStatusAborted
                                         */
            {BFCP_ACT_CONNECTED,
             &BFCP_fsm::
                 BFCPFSM_FloorRequestStatusGranted}, /* BFCP_ACT_FloorRequestStatusGranted
                                         */
            {BFCP_ACT_CONNECTED,
             &BFCP_fsm::BFCPFSM_NullFunction}, /* BFCP_ACT_UserStatus   */
            {BFCP_ACT_CONNECTED,
        ...
        /* from BFCP_ACT_FloorRequest */
        {
            ...
            {BFCP_ACT_FloorRequest,
             &BFCP_fsm::BFCPFSM_NullFunction}, /* BFCP_ACT_CONNECTED */
            {BFCP_ACT_FloorRequest,
             &BFCP_fsm::BFCPFSM_UpdateHwnd}, /* BFCP_ACT_FloorRequest */
        ...
        /* from BFCP_ACT_FloorRelease */
        {
            ...
            {BFCP_ACT_CONNECTED,
             &BFCP_fsm::
                 BFCPFSM_FloorReleaseStatusAborted}, /* BFCP_ACT_FloorRequestStatusAborted
                                                      */
            {BFCP_ACT_CONNECTED,
             &BFCP_fsm::
                 BFCPFSM_FloorRequestStatusGranted}, /* BFCP_ACT_FloorRequestStatusGranted
...
bool BFCP_Participant::CloseUdpConnection() {
    removeSession();
    return true;
}

/* Create a new Participant */
bool BFCP_Participant::OpenUdpConnection(const char *local_address,
                                         int local_port,
    ...
    try {
        Log(INF,
            "BFCP_Participant:: OpenUdpConnection conferenceID[%d] userID[%d] "
            "floorID[%d] server[%s] network [%s:%d]<=>[%s:%d] ",
            m_bfcp_participant_information->conferenceID,
            m_bfcp_participant_information->userID,
            m_bfcp_participant_information->pfloors
                ? m_bfcp_participant_information->pfloors->floorID
                : 0,
            "FALSE", local_address ? local_address : "",
            local_port ? local_port : 0, remote_address ? remote_address : "",
            remote_port ? remote_port : 0);

        SelLocalConnection(local_address, local_port, BFCPConnectionRole::ACTIVE);
...
bool BFCP_Participant::OpenTcpConnection(const char *local_address,
    ...
    try {
        Log(INF,
            "BFCP_Participant:: OpenTcpConnection conferenceID[%d] userID[%d] "
            "floorID[%d] server[%s] network [%s:%d]<=>[%s:%d] ",
            m_bfcp_participant_information->conferenceID,
            m_bfcp_participant_information->userID,
            m_bfcp_participant_information->pfloors
                ? m_bfcp_participant_information->pfloors->floorID
                : 0,
            isServer ? "TRUE" : "FALSE", local_address ? local_address : "",
            local_port ? local_port : 0, remote_address ? remote_address : "",
            remote_port ? remote_port : 0);
            ...
            if (remote_address != NULL && remote_port != 0) {
                if (setRemoteEndpoint(remote_address, remote_port)) {
                    if (connect()) {
                        ...
                        removeSession();
                        Log(ERR,
                            "BFCP_Participant:: Connection connect failed ");
                        // throw IOException("BFCP_Participant:: Connection
                        // connect failed");
                    }
                } else {
...
int BFCP_Participant::bfcp_destroy_bfcp_participant(
    st_bfcp_participant_information **participantp) {
    if (participantp == NULL) return -1;
...
int BFCP_Participant::bfcp_insert_floor_participant(
    st_bfcp_participant_information *participant, UINT16 floorID) {
    if (floorID <= 0) return -1;
    if (participant == NULL) participant = m_bfcp_participant_information;

    bfcp_floors_participant *tmp_floor =
        insert_floor_list(participant->pfloors, floorID, NULL);
    if (tmp_floor) participant->pfloors = tmp_floor;

    return 0;
}

/* Delete a floor from the list of floors the participant is aware of */
int BFCP_Participant::bfcp_delete_floor_participant(
    st_bfcp_participant_information *participant, UINT16 floorID) {
    if (floorID <= 0) return -1;
    if (participant == NULL) participant = m_bfcp_participant_information;
...
/* Hello */
int BFCP_Participant::bfcp_hello_participant(
    st_bfcp_participant_information *participant) {
    if (participant == NULL) participant = m_bfcp_participant_information;
    ...
    arguments->entity = bfcp_new_entity(
        participant->conferenceID, m_base_transactionID, participant->userID);
...
int BFCP_Participant::bfcp_floorStatus_floorRequestStatus_Ack(
    e_bfcp_primitives primitive, UINT32 ConferenceID, UINT16 userID,
    ...
    arguments->primitives = bfcp_new_supported_list(
        ...
        e_primitive_FloorQuery, e_primitive_FloorStatus, e_primitive_Hello,
        e_primitive_HelloAck, e_primitive_Goodbye, e_primitive_GoodbyeAck,
        e_primitive_Error, 0);
...
/* HelloAck */
int BFCP_Participant::bfcp_helloAck_participant(UINT32 ConferenceID,
                                                UINT16 userID,
                                                UINT16 TransactionID) {
    int error;
    bfcp_arguments *arguments;
...
int BFCP_Participant::bfcp_goodbyeAck_participant(UINT32 ConferenceID,
                                                  UINT16 userID,
                                                  UINT16 TransactionID) {
    int error;
    bfcp_arguments *arguments;
...
int BFCP_Participant::bfcp_floorRequest_participant(UINT16 p_floorID) {
    st_bfcp_participant_information *participant =
        m_bfcp_participant_information;
    ...
    bfcp_floors_participant *tmp_floor =
        insert_floor_list(floor_list, p_floorID, NULL);
    if (tmp_floor) floor_list = tmp_floor;

    int error = bfcp_floorRequest_participant(participant, beneficiaryID,
                                              priority, floor_list, NULL);
    if ((error == -1) && (floor_list != NULL)) {
        /* Free the request */
        Log(ERR,
            "Error sending the message: do the floors in the request exist?  error:%d", error);
...
/* FloorRequest */
int BFCP_Participant::bfcp_floorRequest_participant(
    st_bfcp_participant_information *participant, UINT16 beneficiaryID,
    e_bfcp_priority priority, bfcp_floors_participant *list_floors,
    char *participant_info) {
    if (participant == NULL) participant = m_bfcp_participant_information;
    if (priority > BFCP_HIGHEST_PRIORITY) priority = BFCP_HIGHEST_PRIORITY;
    if (list_floors == NULL) {
    ...
    Log(INF,
        "bfcp_floorRequest_participant: find  participant floors[0x%p] "
        "participant floorID[%d] request floorID[%d]",
        participant->pfloors,
        participant->pfloors ? participant->pfloors->floorID : 0,
        list_floors ? list_floors->floorID : 0);
    ...
    for (tempnode = list_floors; tempnode; tempnode = tempnode->next) {
        ...
        while (temp_list_floors &&
               (temp_list_floors->floorID > tempnode->floorID)) {
            Log(INF,
                "bfcp_floorRequest_participant: check  participant floors[%p] "
                "participant floorID[%d] request floorID[%d]",
                temp_list_floors,
                temp_list_floors ? temp_list_floors->floorID : 0,
                tempnode ? tempnode->floorID : 0);
            temp_list_floors = temp_list_floors->next;
        }
        if ((temp_list_floors == NULL) ||
            (tempnode->floorID != temp_list_floors->floorID)) {
            /* This floor is not in the list */
            Log(ERR,
                "bfcp_floorRequest_participant: This floor is not in the list "
                "temp_list_floors[%p] tempnode->floorID[%d] "
                "temp_list_floors->floorID[%d] ",
                temp_list_floors, tempnode ? tempnode->floorID : 0,
                temp_list_floors ? temp_list_floors->floorID : 0);
            return -1;
        }
    }
    ...
    if (participant_info != NULL) {
        ...
        if (dLen != 0) {
            ...
            if (arguments->pInfo == NULL) {
                bfcp_mutex_unlock(count_mutex);
				Log(ERR, "arguments->pInfo is NULL ");
                return -1;
            }
    ...
    /* Send the message to the FCS */
    error = sendBFCPmessage(m_PartSocket, message);
    Log(ERR, "Send BFCP message sock:%d, error code: %d\n", m_PartSocket, error);
    BFCP_SEND_CHECK_ERRORS();
...
int BFCP_Participant::bfcp_floorRelease_participant() {
    ...
    Log(INF, "BFCP_Participant:: created floorRelease FloorRequestID[%d] ",
        m_currentFloorRequestID);

    int error =
        bfcp_floorRelease_participant(participant, m_currentFloorRequestID);
    return error;
}
...
int BFCP_Participant::bfcp_Goodbye_participant() {
    ...
    Log(INF, "BFCP_Participant:: created Goodbye FloorRequestID[%d] ",
        m_currentFloorRequestID);
...
int BFCP_Participant::bfcp_Goodbye_participant(
    st_bfcp_participant_information *participant, UINT16 floorRequestID) {
    if (participant == NULL) participant = m_bfcp_participant_information;
...
/* FloorRequestQuery */
int BFCP_Participant::bfcp_floorRequestQuery_participant(
    st_bfcp_participant_information *participant, UINT16 floorRequestID) {
    if (participant == NULL) participant = m_bfcp_participant_information;
...
/* UserQuery */
int BFCP_Participant::bfcp_userQuery_participant(
    st_bfcp_participant_information *participant, UINT16 beneficiaryID) {
    if (participant == NULL) participant = m_bfcp_participant_information;
...
/* UserQuery */
int BFCP_Participant::bfcp_floorQuery_participant(
    st_bfcp_participant_information *participant,
    bfcp_floors_participant *list_floors) {
    if (participant == NULL) participant = m_bfcp_participant_information;
...
int BFCP_Participant::bfcp_chairAction_participant(
    st_bfcp_participant_information *participant, UINT16 floorRequestID,
    char *statusInfo, UINT16 status, bfcp_floors_participant *list_floors,
    UINT16 queue_position) {
    if (participant == NULL) participant = m_bfcp_participant_information;
    if (list_floors == NULL) return -1;
    if (floorRequestID <= 0) return -1;
    if ((status != BFCP_ACCEPTED) && (status != BFCP_DENIED) &&
        (status != BFCP_REVOKED))
        return -1;
    ...
    if (status != BFCP_ACCEPTED) queue_position = 0;

    floors_IDs = list_floors;
    if (floors_IDs != NULL)
        fRS = bfcp_new_floor_request_status(floors_IDs->floorID, status,
                                            queue_position, floors_IDs->sInfo);
    if (fRS == NULL) {
        bfcp_mutex_unlock(count_mutex);
        return -1;
    }
    ...
    while (floors_IDs != NULL) {
        fRS_temp = bfcp_new_floor_request_status(
            floors_IDs->floorID, status, queue_position, floors_IDs->sInfo);
        if (fRS_temp != NULL)
            bfcp_list_floor_request_status(fRS, fRS_temp, NULL);
        floors_IDs = floors_IDs->next;
    }

    oRS = bfcp_new_overall_request_status(floorRequestID, status,
                                          queue_position, statusInfo);

    arguments->frqInfo = bfcp_new_floor_request_information(
        floorRequestID, oRS, fRS, NULL, NULL, BFCP_NORMAL_PRIORITY, NULL);
...
st_bfcp_floors_participant *BFCP_Participant::create_floor_list(
    UINT16 floorID, char *status_info) {
    bfcp_floors_participant *floor_list;
    size_t dLen = 0;

    /* Allocate a new element */
    floor_list =
        (bfcp_floors_participant *)calloc(1, sizeof(bfcp_floors_participant));
    if (floor_list == NULL)
        return NULL;
        ...
        if (status_info != NULL) {
            ...
            if (dLen != 0) {
                ...
                if (floor_list->sInfo == NULL) return NULL;
...
bfcp_floors_participant *BFCP_Participant::insert_floor_list(
    st_bfcp_floors_participant *floor_list, UINT16 floorID, char *status_info) {
    ...
    /* Allocate a new element */
    floor = (st_bfcp_floors_participant *)calloc(
        1, sizeof(st_bfcp_floors_participant));
    if (floor == NULL) {
    ...
    if (floor_list == NULL) {
        ...
        if (status_info != NULL) {
            ...
            if (dLen != 0) {
                ...
                if (floor->sInfo == NULL) return NULL;
        ...
        Log(INF, "floor_list start with floorID %d insert before FloorID %d ",
            floor_list->floorID, floorID);
        floor->floorID = floorID;
        if (status_info != NULL) {
            /* If there's Status Info text, add it */
        ...
        ini_floor_list = floor_list;
        while (ini_floor_list->next &&
               (ini_floor_list->next->floorID > floorID))
            ini_floor_list = ini_floor_list->next;

        Log(INF, "floor_list after FloorID %d new FloorID[%d]",
            floor_list->floorID, floorID);
    ...
    Log(INF, "insert_floor_list floorID[%d] success ! floor_list[%p]", floorID,
        floor_list);
    return floor_list;
}

/* Destroy a list of 'bfcp_floors_participant' elements */
int BFCP_Participant::remove_floor_list(
    st_bfcp_floors_participant *floor_list) {
...
/*-----------------------------------------------------------------------------------------*/
int BFCP_Participant::ProcessBFCPmessage(bfcp_received_message *m,
                                         BFCP_SOCKET s) {
...
/* Callback to receive notifications from the underlying library about incoming
 * BFCP messages */
bool BFCP_Participant::bfcp_received_msg(bfcp_received_message *recv_msg) {
    ...
    /* Output will be different according to the BFCP primitive in the message
     * header */
    ...
    switch (recv_msg->primitive) {
        case e_primitive_Error:
            Log(INF, "Error:");
            if (!recv_msg->arguments->error) break;
            Log(INF, "Error n.    %d", recv_msg->arguments->error->code);
            Log(INF, "Error info: %s",
                recv_msg->arguments->eInfo ? recv_msg->arguments->eInfo
                                           : "No info");
            evt.Error_codes =
                (e_bfcp_error_codes)recv_msg->arguments->error->code;
        ...
        case e_primitive_Hello:
            ...
            bfcp_helloAck_participant(evt.conferenceID, evt.userID,
                                      evt.TransactionID);
            BFCPFSM_FsmEvent(&evt);
            break;
        case e_primitive_HelloAck:
            ...
            while (info_primitives != NULL) {
                const char* pStr = getBfcpDescPrimitive((e_bfcp_primitives)info_primitives->element);
                if (pStr) {
                    log_str += " ";
                    log_str += pStr;
                }
            ...
            Log(INF, "Supported Primitives: %s\n", log_str.c_str());
            log_str.clear();
            info_attributes = recv_msg->arguments->attributes;
            while (info_attributes != NULL) {
                const char* pStr = getBfcpAttribute((e_bfcp_attibutes)info_attributes->element);
                if (pStr) {
                    log_str += " ";
                    log_str += pStr;
                }
            ...
            Log(INF, "Supported Attributes: %s\n", log_str.c_str());
            BFCPFSM_FsmEvent(&evt);
            break;
        ...
        case e_primitive_FloorStatus:
            ...
            if (recv_msg->arguments->frqInfo) {
                ...
                while (tempInfo) {
                    ...
                    if (tempInfo->oRS) {
                        ...
                        if (tempInfo->oRS->rs) {
                            Log(INF, "      Queue Position  %d",
                                tempInfo->oRS->rs->qp);
                            Log(INF, "      RequestStatus   %s",
                                getBfcpStatus(tempInfo->oRS->rs->rs));
                            evt.Status = (e_bfcp_status)tempInfo->oRS->rs->rs;
                            evt.QueuePosition = tempInfo->oRS->rs->qp;
                        }
                        if (tempInfo->oRS->sInfo)
                            Log(INF, "      Status Info:   %s",
                                tempInfo->oRS->sInfo);
                    }
                    if (tempInfo->fRS) {
                    ...
                    if (tempInfo->pInfo)
                        Log(INF, "    PARTICIPANT PROVIDED INFO:  %s",
                            tempInfo->pInfo);
                    Log(INF, "---");
            ...
            if ((evt.Status != BFCP_PENDING) && (evt.Status != BFCP_ACCEPTED)) {
                bfcp_floorStatus_floorRequestStatus_Ack(
                    recv_msg->primitive, conferenceID, userID, transactionID);
            }
            BFCPFSM_FsmEvent(&evt);
            Log(INF, "");
            break;
        case e_primitive_UserStatus:
            ...
            if (recv_msg->arguments->beneficiary) {
                Log(INF, "BeneficiaryInformation %d:",
            ...
            while (tempInfo) {
                ...
                if (tempInfo->oRS) {
                    ...
                    if (tempInfo->oRS->sInfo)
                        Log(INF, "      Status Info:   %s",
                            tempInfo->oRS->sInfo ? tempInfo->oRS->sInfo
                                                 : "No info");
                }
                if (tempInfo->beneficiary) {
        ...
        case e_primitive_Goodbye:
            ...
            bfcp_goodbyeAck_participant(evt.conferenceID, evt.userID,
                                        evt.TransactionID);
            BFCPFSM_FsmEvent(&evt);
            break;
        default:
...
bool BFCP_Participant::checkFloorRequestID(UINT16 p_FloorRequestID) {
    Log(INF,
        "checkFloorRequestID current FloorRequestID[%d] new FloorRequestID[%d]",
        m_currentFloorRequestID, p_FloorRequestID);

    // if ( ( m_currentFloorRequestID && p_FloorRequestID !=
    // m_currentFloorRequestID ) ){
    //     Log(ERR,"Error current FloorRequestID[%d] new FloorRequestID[%d]
    //     ",m_currentFloorRequestID,p_FloorRequestID); return false ;
    // }
    m_currentFloorRequestID = p_FloorRequestID;
    return true;
}
...
// FSM
bool BFCP_Participant::BFCPFSM_UpdatesBfcpFsmEvent(s_bfcp_msg_event *p_bfcp_evt,
                                                   e_BFCP_ACT p_evt,
                                                   e_BFCP_ACT p_st) {
    ...
    if (Status) {
        ...
        if (p_bfcp_evt) {
            ...
            strncpy(bfcpFsmEvt.c_param, p_bfcp_evt->c_param,
                    sizeof(bfcpFsmEvt.c_param));
            bfcpFsmEvt.p_bfcp_evt = p_bfcp_evt;
        } else {
            if (m_bfcp_participant_information) {
                bfcpFsmEvt.userID = m_bfcp_participant_information->userID;
                bfcpFsmEvt.FloorID =
                    m_bfcp_participant_information->pfloors->floorID;
                bfcpFsmEvt.conferenceID =
                    m_bfcp_participant_information->conferenceID;
            }
        }
...
bool BFCP_Participant::OnBFCPConnected(BFCP_SOCKET socket, const char *remoteIp,
                                       int remotePort) {
    Log(INF, _T("BFCP connection callback sock fd [%d] [%s:%d]"), socket,
        remoteIp, remotePort);
    m_PartSocket = socket;
    s_bfcp_msg_event evt;
    memset(&evt, 0, sizeof(s_bfcp_msg_event));
    if (remoteIp) strncpy(evt.c_param, remoteIp, BFCP_STRING_SIZE);
    if (remotePort) evt.i_parm = remotePort;
    return FsmCtrlPerform(BFCP_fsm::BFCP_ACT_CONNECTED, &evt);
}

bool BFCP_Participant::OnBFCPDisconnected(BFCP_SOCKET socket) {
    Log(INF, _T("BFCP disconnection callback sock fd [%d] [%s:%d]"), socket, __FUNCTION__, __LINE__);
    s_bfcp_msg_event evt;
    memset(&evt, 0, sizeof(s_bfcp_msg_event));
...
bool BFCP_Participant::BFCPFSM_FloorRequestStatusGranted(
    s_bfcp_msg_event *p_evt) {
    if (!p_evt) return false;
    Log(INF,
        _T("BFCP floorRequestStatusGranted. myFloorId[%d] eventFloorId[%d]"),
        m_bfcp_participant_information->pfloors->floorID, p_evt->FloorID);
    if (!p_evt->FloorID ||
        m_bfcp_participant_information->pfloors->floorID == p_evt->FloorID) {
        return true;
    } else {
        Log(ERR,
            _T("BFCP floorRequestStatusGranted. myFloorId[%d] != ")
            _T("eventFloorId[%d]"),
            m_bfcp_participant_information->pfloors->floorID, p_evt->FloorID);
        return false;
    }
...
bool BFCP_Participant::BFCPFSM_FloorRequestStatusAborted(
    s_bfcp_msg_event *p_evt) {
    if (!p_evt) return false;
    Log(INF,
        _T("BFCP floorRequestStatusAborted. myFloorId[%d] eventFloorId[%d]"),
        m_bfcp_participant_information->pfloors->floorID, p_evt->FloorID);
    if (!p_evt->FloorID ||
        m_bfcp_participant_information->pfloors->floorID == p_evt->FloorID) {
        return true;
        ...
        Log(ERR,
            _T("BFCP floorRequestStatusAborted. myFloorId[%d] != ")
            _T("eventFloorId[%d]"),
            m_bfcp_participant_information->pfloors->floorID, p_evt->FloorID);
        return false;
    }
...
bool BFCP_Participant::BFCPFSM_FloorRequestStatusAccepted(
    s_bfcp_msg_event *p_evt) {
    if (!p_evt) return false;
    Log(INF,
        _T("BFCP floorRequestStatusAccepted. myFloorId[%d] eventFloorId[%d] ")
        _T("Status[%s]"),
        m_bfcp_participant_information->pfloors->floorID, p_evt->FloorID,
        getBfcpStatus(p_evt->Status));
    if (!p_evt->FloorID ||
        m_bfcp_participant_information->pfloors->floorID == p_evt->FloorID) {
        return true;
    } else {
        Log(ERR,
            _T("BFCP floorRequestStatusAccepted. myFloorId[%d] != ")
            _T("eventFloorId[%d]"),
            m_bfcp_participant_information->pfloors->floorID, p_evt->FloorID);
        return false;
    }
...
bool BFCP_Participant::BFCPFSM_FloorReleaseStatusAborted(
    s_bfcp_msg_event *p_evt) {
    if (!p_evt) return false;
    Log(INF,
        _T("BFCP floorReleaseStatusAborted. myFloorId[%d] eventFloorId[%d] ")
        _T("Status[%s]"),
        m_bfcp_participant_information->pfloors->floorID, p_evt->FloorID,
        getBfcpStatus(p_evt->Status));
    if (m_bfcp_participant_information->pfloors->floorID == p_evt->FloorID) {
        ...
        Log(ERR,
            _T("BFCP floorReleaseStatusAborted. myFloorId[%d] != ")
            _T("eventFloorId[%d]"),
            m_bfcp_participant_information->pfloors->floorID, p_evt->FloorID);
        return false;
    }
...
bool BFCP_Participant::BFCPFSM_FloorStatusAccepted(s_bfcp_msg_event *p_evt) {
    if (!p_evt) return false;
    Log(INF,
        _T("BFCP FloorStatusAccepted. myFloorId[%d] eventFloorId[%d] ")
        _T("Status[%s]"),
        m_bfcp_participant_information->pfloors->floorID, p_evt->FloorID,
        getBfcpStatus(p_evt->Status));
    if (m_bfcp_participant_information->pfloors->floorID == p_evt->FloorID) {
...
bool BFCP_Participant::BFCPFSM_FloorStatusGranted(s_bfcp_msg_event *p_evt) {
    if (!p_evt) return false;
    Log(INF,
        _T("BFCP FloorStatusGranted. myFloorId[%d] eventFloorId[%d] ")
        _T("BeneficiaryID[%d] Status[%s]"),
        m_bfcp_participant_information->pfloors->floorID, p_evt->FloorID,
        p_evt->BeneficiaryID, getBfcpStatus(p_evt->Status));
    if (m_bfcp_participant_information->pfloors->floorID == p_evt->FloorID) {
        return true;
    } else {
        Log(ERR,
...
bool BFCP_Participant::BFCPFSM_FloorStatusAborted(s_bfcp_msg_event *p_evt) {
    if (!p_evt) return false;
    Log(INF,
        _T("BFCP FloorRequestAborted. myFloorId[%d] eventFloorId[%d] ")
        _T("Status[%s]"),
        m_bfcp_participant_information->pfloors->floorID, p_evt->FloorID,
        getBfcpStatus(p_evt->Status));
    if (m_bfcp_participant_information->pfloors->floorID == p_evt->FloorID) {
...
bool BFCP_Participant::BFCPFSM_FloorRequest(s_bfcp_msg_event *p_evt) {
    bool status = false;
    if (bfcp_floorRequest_participant(
            m_bfcp_participant_information->pfloors->floorID) == -1) {
...
void BFCP_Participant::Log(const char *pcFile, int iLine, int iErrLevel,
                           const char *pcFormat, ...) {
    va_list arg;
    va_start(arg, pcFormat);
    if (m_ParticipantEvent)
