    ...
    st_BFCP_fsm BFCPStateMachine[BFCP_ACT_LAST][BFCP_ACT_LAST] = {
        ...
        /* from BFCP_ACT_CONNECTED */
        {
            ...
            {BFCP_ACT_SERVER_SHARE,
             &BFCP_fsm::BFCPFSM_FloorStatusGranted}, /* BFCP_ACT_FloorStatusGranted
                                                      */
            {BFCP_ACT_CONNECTED,
             &BFCP_fsm::BFCPFSM_NullFunction}, /* BFCP_ACT_ChairActionAck   */
            {BFCP_ACT_CONNECTED,
        ...
        /* from BFCP_ACT_FloorRequest */
        {
            ...
            {BFCP_ACT_FloorRequest,
             &BFCP_fsm::BFCPFSM_NullFunction}, /* BFCP_ACT_Error  */
            {BFCP_ACT_FloorRequest,
             &BFCP_fsm::BFCPFSM_FloorRequestStatusAccepted}, /* BFCP_ACT_FloorRequestStatusAccepted
                                                              */
            {BFCP_ACT_CONNECTED,
             &BFCP_fsm::BFCPFSM_FloorRequestStatusAborted}, /* BFCP_ACT_FloorRequestStatusAborted
                                                             */
            {BFCP_ACT_SHARE,
             &BFCP_fsm::BFCPFSM_FloorRequestStatusGranted}, /* BFCP_ACT_FloorRequestStatusGranted
                                                             */
            {BFCP_ACT_FloorRequest,
             &BFCP_fsm::BFCPFSM_NullFunction}, /* BFCP_ACT_UserStatus   */
            {BFCP_ACT_FloorRequest,
             &BFCP_fsm::BFCPFSM_FloorStatusAccepted}, /* BFCP_ACT_FloorStatusAccepted
                                                       */
            {BFCP_ACT_FloorRequest,
             &BFCP_fsm::BFCPFSM_FloorStatusAborted}, /* BFCP_ACT_FloorStatusAborted
                                                      */
            {BFCP_ACT_SERVER_SHARE,
             &BFCP_fsm::BFCPFSM_FloorStatusGranted}, /* BFCP_ACT_FloorStatusGranted
                                                      */
        ...
        /* from BFCP_ACT_FloorRelease */
        {
            ...
            {BFCP_ACT_FloorRelease,
             &BFCP_fsm::BFCPFSM_FloorRequestStatusAccepted}, /* BFCP_ACT_FloorRequestStatusAccepted
                                                              */
            {BFCP_ACT_CONNECTED,
             &BFCP_fsm::BFCPFSM_FloorReleaseStatusAborted}, /* BFCP_ACT_FloorRequestStatusAborted
                                                             */
            {BFCP_ACT_CONNECTED,
             &BFCP_fsm::BFCPFSM_FloorRequestStatusGranted}, /* BFCP_ACT_FloorRequestStatusGranted
                                                             */
        ...
        /* from BFCP_ACT_FloorRequestStatusAccepted*/
        {
            ...
            {BFCP_ACT_FloorRequestStatusAccepted,
             &BFCP_fsm::BFCPFSM_NullFunction}, /* BFCP_ACT_Error  */
            {BFCP_ACT_FloorRequestStatusAccepted,
             &BFCP_fsm::BFCPFSM_NullFunction}, /* BFCP_ACT_FloorRequestStatusAccepted
                                                */
            {BFCP_ACT_FloorRequestStatusAccepted,
             &BFCP_fsm::BFCPFSM_NullFunction}, /* BFCP_ACT_FloorRequestStatusAborted
                                                */
            {BFCP_ACT_FloorRequestStatusAccepted,
             &BFCP_fsm::BFCPFSM_NullFunction}, /* BFCP_ACT_FloorRequestStatusGranted
                                                */
            {BFCP_ACT_FloorRequestStatusAccepted,
             &BFCP_fsm::BFCPFSM_NullFunction}, /* BFCP_ACT_UserStatus   */
            {BFCP_ACT_FloorRequestStatusAccepted,
...
bool BFCP_fsm::FsmCtrlPerform(e_BFCP_ACT p_evt, s_bfcp_msg_event* p_bfcp_evt) {
    /*
     * Note(hsu)
     *  根据日志"FSM [Server] current state[connected] event[Hello] expected state[connected] ... perform action"
     *  1.  m_BFCPStateMachine第一层数组表示各个状态(e_BFCP_ACT)针对不同事件(e_BFCP_ACT)的handler，
     *      不同的索引分别代表了不同的e_BFCP_ACT值。
     *  2.  m_BFCPStateMachine第二层数组的索引表示p_evt的整型值。
     *  3.  第二层数组的元素中，st_BFCP_fsm::new_state表示该事件后的最终状态；
     *      st_BFCP_fsm::function表示该事件的handler
     */
    e_BFCP_ACT new_bfcpFsmSt = m_BFCPStateMachine[m_bfcpFsmSt][p_evt].new_state;
    Log(INF, "FSM [%s] current state[%s] event[%s] from sockfd [%d] expected state[%s] ... perform action", m_name,
        getBfcpFsmAct(m_bfcpFsmSt), getBfcpFsmAct(p_evt),
        p_bfcp_evt->sockfd, getBfcpFsmAct(new_bfcpFsmSt));
    bool Status =
        (*this.*m_BFCPStateMachine[m_bfcpFsmSt][p_evt].function)(p_bfcp_evt);
    if (Status) {
        Log(INF,
            "FSM [%s] action sucessful. Current state [%s] => new state [%s]", m_name,
        ...
        if (strcmp(getBfcpFsmAct(p_evt), "FloorStatusGranted") != 0)
            Log(INF,
                "FSM [%s] event %s and status %s NOT successful, state will "
                "not change",
                getBfcpFsmAct(m_bfcpFsmSt), getBfcpFsmAct(p_evt),
                getBfcpStatus(p_bfcp_evt->Status));
        else {
            Log(INF, "Providing callback for event %s and status %s ",
                getBfcpFsmAct(p_evt), getBfcpStatus(p_bfcp_evt->Status));
            BFCPFSM_UpdatesBfcpFsmEvent(p_bfcp_evt, p_evt, m_bfcpFsmSt);
        }
    }
    return Status;
...
bool BFCP_fsm::BFCPFSM_NullFunction(s_bfcp_msg_event* p_evt) {
    if (p_evt) {
        Log(WAR,
            _T("BFCP FSM current state[%s] Status[%s] eventFloorId[%d] => No ")
            _T("action"),
            getBfcpFsmAct(GetFSMState()),
            p_evt->Status ? getBfcpStatus(p_evt->Status) : "", p_evt->FloorID);
    } else {
        Log(WAR,
            _T("BFCP FSM current state[%s] received NULL event => No action"),
            getBfcpFsmAct(GetFSMState()));
    }
    return false;
}
...
bool BFCP_fsm::BFCPFSM_FsmEvent(s_bfcp_msg_event* p_bfcpEvent) {
    bool status = false;
    if (!p_bfcpEvent) return status;
    Log(INF, _T("BFCPFSM_FsmEvent [%d / %s] status[%s] "), p_bfcpEvent->Event,
        getBfcpDescPrimitive(p_bfcpEvent->Event),
    ...
    switch (p_bfcpEvent->Event) {
        ...
        case e_primitive_FloorRequestStatus:
            switch (p_bfcpEvent->Status) {
                case BFCP_PENDING:
                case BFCP_ACCEPTED:
                    return FsmCtrlPerform(BFCP_ACT_FloorRequestStatusAccepted,
                                          p_bfcpEvent);
                case BFCP_GRANTED:
                    return FsmCtrlPerform(BFCP_ACT_FloorRequestStatusGranted,
                                          p_bfcpEvent);
                case BFCP_DENIED:
                case BFCP_CANCELLED:
                case BFCP_RELEASED:
                case BFCP_REVOKED:
                    return FsmCtrlPerform(BFCP_ACT_FloorRequestStatusAborted,
                                          p_bfcpEvent);
                default:
                    Log(ERR,
                        _T("BFCPFSM_FsmEvent unknown Event[%d] status[%d]"),
                        p_bfcpEvent->Event, p_bfcpEvent->Status);
                    return false;
            }
        ...
        case e_primitive_FloorStatus:
            switch (p_bfcpEvent->Status) {
                case BFCP_PENDING:
                case BFCP_ACCEPTED:
                    return FsmCtrlPerform(BFCP_ACT_FloorStatusAccepted,
                                          p_bfcpEvent);
                case BFCP_GRANTED:
                    return FsmCtrlPerform(BFCP_ACT_FloorStatusGranted,
                                          p_bfcpEvent);
                case BFCP_DENIED:
                case BFCP_CANCELLED:
                case BFCP_RELEASED:
                case BFCP_REVOKED:
                    return FsmCtrlPerform(BFCP_ACT_FloorStatusAborted,
                                          p_bfcpEvent);
                default:
                    Log(ERR,
                        _T("BFCPFSM_FsmEvent unknown Event[%d] status[%d]"),
        ...
        default:
            Log(ERR, _T("BFCPFSM_FsmEvent unknown Event[%d] status[%d]"),
                p_bfcpEvent->Event, p_bfcpEvent->Status);
            return false;
    }
}

void BFCP_fsm::InitFsmEvent(st_BFCP_fsm_event* p_fsm_evt, e_BFCP_ACT p_state,
                            const s_bfcp_msg_event* p_bfcp_evt) {
    ...
    if (p_bfcp_evt) {
        ...
        strncpy(p_fsm_evt->c_param, p_bfcp_evt->c_param,
                sizeof(p_fsm_evt->c_param));
    }
}
