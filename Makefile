...
ifeq ($(qn_cross_compile), yes)
	COMMON_CC_OPTS  = -fPIC -Wall -Werror -Wunused -Wstrict-prototypes -Wmissing-prototypes -Wmissing-declarations -DQN_CROSS_COMPILE -DHSU=1 -DOPAL_BFCP=1
	COMMON_CPP_OPTS = -fPIC -Wall -Werror -Wunused -DQN_CROSS_COMPILE -DHSU=1 -DOPAL_BFCP=1
else
	COMMON_CC_OPTS  = -fPIC -Wall -Werror -Wunused -Wstrict-prototypes -Wmissing-prototypes -Wmissing-declarations -Wno-error=stringop-truncation -DHSU=1 -DOPAL_BFCP=1
	COMMON_CPP_OPTS = -fPIC -Wall -Werror -Wunused -Wno-error=stringop-truncation -DHSU=1 -DOPAL_BFCP=1
endif


ifeq ($(DEBUG),yes)
       MAIN_CC_OPTS = -D__DEBUG__ -g $(COMMON_CC_OPTS)
       MAIN_CPP_OPTS = -D__DEBUG__ -g $(COMMON_CPP_OPTS)
       MAIN_LIBS = -g -lpthread
       BINS_PRE=dbg
else
       MAIN_CC_OPTS = -O2 $(COMMON_CC_OPTS)
       MAIN_CPP_OPTS = -O2 $(COMMON_CPP_OPTS)
       MAIN_LIBS = -lpthread
       BINS_PRE=rel
endif

# qn_cross_compiler_prefix=aarch64-linux-android-
MAIN_LIBNAME=libbfcp$(BINS_PRE)
MAIN_CC = ${qn_cross_compiler_prefix}gcc
MAIN_CPP = ${qn_cross_compiler_prefix}g++
MAIN_AR = ${qn_cross_compiler_prefix}ar
